            ...
            WebDriver driver = new FirefoxDriver(options);
                ...
                for(int i=1;i<=PAGE_NUMBER;i++){
                    ...
                    for(Element e:elements){
                        ...
                        String detailLink = "https://news.bitcoin.com" +e.selectFirst("a").attr("href");
                        String img = e.select("img").attr("src");
                        String title = e.select("h6").text();
                        String dateContent = e.select("p[class='sc-dSJDGZ fBa-dYU']").text();
                        String datePart = extractDate(dateContent);
                        postData.put("detailLink", detailLink);
                        postData.put("date", datePart);
                        postData.put("img", img);
                        postData.put("title", title);
                        crawTags(detailLink,driver,postData,pageDataMap);
                        dem++;
                    }
                }
...
public void crawTags(String link,WebDriver driver,JSONObject postData,Map<String, JSONObject> pageDataMap){
    driver.get(link);
    try {
        sleep(3000);
    } catch (InterruptedException e) {
    ...
    public static void main (String[]args){
        NewBitcoinsScraper newBitcoinsScraper = new NewBitcoinsScraper();
        newBitcoinsScraper.scrape();
    }
